...
namespace AdventOfCode2022
{
    public static class Day17
    {
        ...
        private static bool RockFalls(List<char[]> chamber, char[,] rock)
        {
            if (!RockCanFall(chamber, rock))
            {
                return false;
            }
            ...
            for (int y = rock.GetLength(1); y > 0 ; y--)
            {
                for (int x = 0; x < ChamberWidth; x++)
                {
                    if (chamber[chamber.Count - y][x] == '@')
                    {
                        chamber[chamber.Count - y][x] = '.';
                        chamber[chamber.Count - (y + 1)][x] = '@';
                    }
                }
            }
        ...
        private static bool RockCanFall(List<char[]> chamber, char[,] rock)
        {
            var rockBaseIndex = chamber.Count - rock.GetLength(1);
